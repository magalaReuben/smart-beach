    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Table <span className='bg-foreground text-primary-foreground rounded px-1'>{tableDelete?.number}</span> will be permanently deleted
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export default function TableTable() {
  ...
  const {data: ListTableData} = useTableListQuery()
  const data = ListTableData?.payload?.data ?? []
      ...
      <div className='w-full'>
        ...
        <div className='flex items-center justify-end space-x-2 py-4'>
          <div className='text-xs text-muted-foreground py-4 flex-1 '>
            Showing <strong>{table.getPaginationRowModel().rows.length}</strong> of <strong>{data.length}</strong> results
          </div>
          <div>
            <AutoPagination
