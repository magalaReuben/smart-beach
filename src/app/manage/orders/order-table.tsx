...
// MakeOrder button is provided at the page level; remove from table to avoid duplicate
...
export const OrderTableContext = createContext({
  ...
  updatingOrderId: null as number | null,
...
export default function OrderTable() {
  ...
  const tableList =
    (tableListQuery.data?.payload.data ?? []) as TableListResType["data"];
  ...
  const changeStatus = async (body: {
    ...
    setUpdatingOrderId(body.orderId);
    try {
  ...
  const handleDeleteOrder = async () => {
    if (!orderDeletingId) return;
    try {
      await deleteOrderMutation.mutateAsync(orderDeletingId);
  ...
  useEffect(() => {
    ...
    function onNewOrder(data: GuestCreateOrdersResType["data"]) {
      const { guest } = data[0];
      toast(`${guest?.name} at table ${guest?.tableNumber} created ${data.length} orders`);
      refetch();
    }

    function onPayment(data: PayGuestOrdersResType["data"]) {
      const { guest } = data[0];
      toast(`${guest?.name} at table ${guest?.tableNumber} paid ${data.length} orders successfully`);
      refetch();
    }
      ...
      <div className="w-full">
        ...
        <div className="flex items-center justify-end space-x-2 py-4">
          <div className="text-xs text-muted-foreground py-4 flex-1 ">
            Showing <strong>{table.getPaginationRowModel().rows.length}</strong> of <strong>{orderList.length}</strong> results
          </div>
          <div>
            <AutoPagination
