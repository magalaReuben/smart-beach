...
import revalidateApi from "@/apis/revalidate";
  ...
  const deleteDish = async () => {
    if (dishDelete) {
      try {
        ...
        toast.success("You have deleted dish successfully!");
        setDishDelete(null);
      } catch (error) {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Dish <span className="bg-foreground text-primary-foreground rounded px-1">{dishDelete?.name}</span> will be permanently deleted
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export default function DishTable() {
  ...
  const { data: dishListData } = useDishListQuery();
  const data = dishListData?.payload?.data ?? [];

  const tableData = data.length === 0 ? MENU_ITEMS : data;
  const [selectedCategory, setSelectedCategory] = useState<string>('All')

  const categories = [
    'All',
    ...Array.from(new Set(tableData.map((item: any) => String(item.category || '')).filter((c: string) => c !== ''))),
  ] as string[]
      ...
      <div className="w-full">
        ...
        <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {data.length === 0
                ? // render static MENU_ITEMS directly when DB is empty
                  (filteredTableData.length ? (
                    filteredTableData.map((item: any) => (
                      <TableRow key={`static-${item.id}`}>
                        ...
                        <TableCell className="capitalize">{formatCurrency(item.price)}</TableCell>
                        <TableCell>
                          <div className="whitespace-pre-line" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(item.description) }} />
                        </TableCell>
                        <TableCell>{getVietnameseDishStatus(item.status)}</TableCell>
                        <TableCell>
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" className="h-8 w-8 p-0">
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuLabel>Actions</DropdownMenuLabel>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => setDishIdEdit(item.id)}>Edit</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => setDishDelete(item as DishItem)}>Delete</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </TableCell>
                      </TableRow>
                ...
                : // render rows from react-table when DB has data
        ...
        <div className="flex items-center justify-end space-x-2 py-4">
          <div className="text-xs text-muted-foreground py-4 flex-1 ">
            Showing <strong>{table.getPaginationRowModel().rows.length}</strong> of <strong>{tableData.length}</strong> results
          </div>
          <div>
            <AutoPagination
