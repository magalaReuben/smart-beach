  ...
  const onSubmit = async (values: CreateDishBodyType) => {
    ...
    try {
      ...
      if (file) {
        ...
        const resImg = await uploadImageMutation.mutateAsync(formData);
        const imageUrl = resImg.payload.data;
                ...
                <div className="col-span-3 w-full space-y-2">
                  <div className="flex gap-2">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex-1">
                          <Input
                            id="category"
                            placeholder="Type new category or pick from suggestions"
                            className="w-full"
                            {...field}
                          />
                          <FormMessage />
                    ...
                    <div className="w-40">
                      <Select
                        onValueChange={(val) => {
                          if (val) form.setValue("category", val);
                        }}
                      >
                        <FormControl>
                          <SelectTrigger>
                        ...
                        <SelectContent>
                            {Array.from(new Set(MENU_ITEMS.map((d: any) => d.category || "Uncategorized"))).map((c) => (
